...
def boar_brawl(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    total = 0
    ones_of_player_score = player_score % 10
    tens_of_opponent_score = opponent_score // 10 % 10
    total = 3 * abs(ones_of_player_score - tens_of_opponent_score)
    if(total == 0):
        return 1
    return total
    # END PROBLEM 2
...
def sus_points(score):
    ...
    "*** YOUR CODE HERE ***"
    def is_sus():
        return num_factors(score) == 3 or num_factors(score) == 4
...
def ceil_div(a, b):
    result = a // b #普通的地板除法
    if a % b != 0: #如果有余数，说明结果不是整数，需要向上舍入
        result += 1
    return result
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def always_roll_n_num(score, opponent_score):
        return n
    return always_roll_n_num
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score0 < goal:
        while score1 < goal:
            if num_of_dice != strategy(score0, score1):
                return False
            score1 += 1
        score0 += 1
        score1 = 0 #清空重新开始
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    boar_score = boar_brawl(score, opponent_score)
    sus_zero_turn_score = sus_points(boar_score + score) - score
    if(sus_zero_turn_score >= threshold):
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
